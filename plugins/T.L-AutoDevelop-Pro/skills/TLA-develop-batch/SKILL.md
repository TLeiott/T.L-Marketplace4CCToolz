...
description: "Fully autonomous batch develop pipeline with scheduler-guided, statusline-aware parallelism. No confirmations, no stops."
argument-hint: [path to tasks.md]
disable-model-invocation: true
---

# /TLA-develop-batch — Vollautonome Geplante Batch-Implementierungs-Pipeline
...
AUSNAHME: Eine einzige Rueckfrage VOR dem ersten Pipeline-Start ist erlaubt, wenn weder Statusline noch Usage-Cache fuer den 5h-Gate verfuegbar sind.
...
- `errors` kurz zeigen
- dem Nutzer GENAU EINMAL vor dem Start die Rueckfrage stellen: "Statusline und Usage-Cache sind nicht verfuegbar. Soll die 5h-Usage fuer diesen Batch ignoriert werden?"
- Bei Nein: abbrechen
...
3. Falls `ok=true`:
   - Wenn `waitedSeconds > 0`: kurz melden, dass der Start fuer diesen Task wegen 5h-Usage pausiert war
...
## STEP 14 — SEQUENZIELL MERGEN + AUTO-COMMITTEN

Pro verbleibendem `ACCEPTED` Task (in geplanter Reihenfolge), vollautomatisch OHNE User-Bestaetigungen:
...
8. Nach erfolgreichem Commit `result.files` zu `mergedFiles` hinzufuegen
